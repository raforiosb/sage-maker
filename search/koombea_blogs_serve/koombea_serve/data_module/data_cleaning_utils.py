...
def convert_date(date: datetime.datetime):
    """str_date _ format = '2014-09-23'"""
    return date.strftime("%b %d, %Y")


def loaded_from_path_clean_df(blogs: pd.DataFrame):
    blogs["post_date"] = pd.to_datetime(blogs["post_date"]).apply(convert_date)
    blogs["post_modified"] = pd.to_datetime(blogs["post_modified"]).apply(convert_date)

    blogs = loaded_from_redis_clean_df(blogs)
    return blogs


def loaded_from_redis_clean_df(blogs: pd.DataFrame):
    blogs["post_excerpt"].fillna("", inplace=True)
    blogs = normalize_columns_for_search_response(blogs)
    return blogs


def normalize_columns_for_search_response(blogs: pd.DataFrame):
    columns_mapping = {
        # features
        "ID": "id",
        "post_name": "slug",
        ...
        "image": "image",
        # intermediate
        "industry_slug": "industry_term",
        "category_slug": "content_type_term",
        "lang": "lang",
    }
    blogs = blogs.rename(columns=columns_mapping)
    return blogs
